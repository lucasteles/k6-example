...
export const getUser = (user: User): Response<User> => {
  ...
  check(res, {
    ...
    'Get User By Id: has correct id': () => jsonRes.data.id === user.id,
...
export const updateUser = (user: User): Response<UpdateUserResponse> => {
  ...
  check(res, {
    ...
    'Update User By Id: has correct updatedAt': () => jsonRes.updatedAt !== '',
    'Update User By Id: has correct job title': () => jsonRes.job === jobTitle,
  })
...
export const deleteUser = (userId: number): Response<object> => {
  const url = reqResUrl
